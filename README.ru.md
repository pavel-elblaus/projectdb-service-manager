...
- **Автоматический запуск с сохранением ручной остановки.** Зарегистрированные приложения работают как службы Windows и продолжают работу без открытого терминала и без необходимости держать пользовательскую сессию. Если приложение явно остановлено в Service Manager, после перезагрузки Windows оно останется остановленным до следующего ручного запуска.
