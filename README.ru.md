...
В блоке **Last activity** отображается последняя структурированная запись лога. Сообщения с признаками предупреждения или ошибки дополнительно выделяются цветом в интерфейсе Service Manager.
...
Установщик запрашивает права администратора при установке и обновлении.

Обычное управление приложениями — **Start**, **Stop**, **Restart** и **Restart all** — работает без отдельного запроса UAC.

При действиях, изменяющих конфигурацию Windows или общую установку ProjectDB, Windows может запросить подтверждение прав администратора. К таким действиям относятся добавление и удаление приложений, управление локальной библиотекой и полная деинсталляция.
