...
- **Автоматический запуск с сохранением ручной остановки.** Службы приложений ProjectDB используют тип запуска Windows `Manual`. Service Manager хранит выбранное пользователем состояние Start/Stop, а системная задача при загрузке Windows запускает только включённые приложения. Если приложение явно остановлено в Service Manager, после перезагрузки оно останется остановленным до следующего ручного запуска.
...
Сами службы приложений ProjectDB специально имеют тип запуска Windows `Manual`. Системная задача `ProjectDB Service Startup` запускается от SYSTEM при загрузке Windows и поднимает только те приложения, для которых Service Manager сохранил включённый автозапуск. `Start` включает это состояние, `Stop` выключает, а `Restart` его не меняет.
